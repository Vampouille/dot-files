...
prompt_battery() {
  ...
  if [[ $(uname) == "Linux" && -d /sys/module/battery ]] ; then
    ...
    BATTERY_ID=$(acpi -bi | grep design | cut -f 1 -d ':')

    function battery_is_charging() {
      ! [[ $(acpi 2&>/dev/null | grep '$BATTERY_ID' |grep -c '^Battery.*Discharging') -gt 0 ]]
    }

    function battery_pct() {
      if (( $+commands[acpi] )) ; then
        echo "$(acpi | grep '$BATTERY_ID' | cut -f2 -d ',' | tr -cd '[:digit:]')"
      fi
    }
    ...
    if [[ $(acpi 2&>/dev/null | grep '$BATTERY_ID' | grep -c '^Battery.*Discharging') -gt 0 ]] ; then
      if [ $b -gt 40 ] ; then
...
RPROMPT="[%D{%H:%M:%S}] $(zsh_command_time)"
